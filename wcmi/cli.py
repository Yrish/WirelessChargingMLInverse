...
def get_default_actions(parser=argument_parser):
	...
	return (
		("train", {
			"dense":      (True,                        "--dense"),
			"load_data":  ("dist/4th_dataset_noid.csv", "--load-data=dist/4th_dataset_noid.csv"),
			"save_model": ("dist/dense.pt",             "--save-model=dist/dense.pt"),
			"save_data":  ("dist/dense_mse.csv",        "--save_data=dist/dense_mse.csv"),
		}),
		("train", {
			"dense":      (True,                        "--dense"),
		...
		("train", {
			...
			"save_data":  ("dist/gan_mse.csv",          "--save_data=dist/gan_mse.csv"),
		}),
		("train", {
			"gan":        (True,                        "--gan"),
		...
		("run", {
			...
			"load_model": ("dist/dense.pt",             "--load-model=dist/dense.pt"),
			"load_data":  ("dist/4th_dataset_noid.csv", "--load-data=dist/4th_dataset_noid.csv"),
			"save_data":  ("dist/dist/4th_dataset_dense_predictions.csv", "--save_data=dist/4th_dataset_dense_predictions.csv"),
		}),
		("run", {
			"gan":        (True,                        "--gan"),
			"load_model": ("dist/gan.pt",               "--load-model=dist/gan.pt"),
			"load_data":  ("dist/4th_dataset_noid.csv", "--load-data=dist/4th_dataset_noid.csv"),
			"save_data":  ("dist/dist/4th_dataset_gan_predictions.csv", "--save_data=dist/4th_dataset_gan_predictions.csv"),
		}),

		("stats", {
			"load_data":  ("dist/dist/4th_dataset_dense_mse.csv", "--load_data=dist/4th_dataset_dense_mse.csv"),
			"save_data":  ("dist/stats",                "--save-data=dist/stats"),
		}),
		("stats", {
			"load_data":  ("dist/dist/4th_dataset_gan_mse.csv", "--load_data=dist/4th_dataset_gan_mse.csv"),
			"save_data":  ("dist/stats",                "--save-data=dist/stats"),
		}),
		("stats", {
			"load_data":  ("dist/dist/4th_dataset_dense_predictions.csv", "--load_data=dist/4th_dataset_dense_predictions.csv"),
			"save_data":  ("dist/stats",                "--save-data=dist/stats"),
		}),
		("stats", {
			"load_data":  ("dist/dist/4th_dataset_gan_predictions.csv", "--load_data=dist/4th_dataset_gan_predictions.csv"),
			"save_data":  ("dist/stats",                "--save-data=dist/stats"),
		}),
	)
